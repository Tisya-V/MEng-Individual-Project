...
def train_domain(domain: str):
    ...
    train_df = pd.read_csv(SPLITS_DIR / f"{domain}_train.csv").dropna(subset=["src_en", "ref_fr"])
    val_df   = pd.read_csv(SPLITS_DIR / f"{domain}_val.csv").dropna(subset=["src_en", "ref_fr"])

    if len(train_df) > MAX_TRAIN_SAMPLES:
        train_df = train_df.sample(n=MAX_TRAIN_SAMPLES, random_state=SEED)
        print(f"  Subsampled train to {MAX_TRAIN_SAMPLES} (seed={SEED})")
